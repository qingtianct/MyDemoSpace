...
@implementation CTPasswordInputView
...
- (NSArray *)dotsArray
{
    if (_dotsArray == nil) {
        ...
        for (int i = 0 ; i < _passwordLength; i ++) {
            ...
            imageView.image = [UIImage ct_circleAndStretchableImageWithColor:self.dotColor size:CGSizeMake(self.dotWidth, self.dotWidth)];
...
- (void)layoutSubviews
{
    ...
    CGFloat gridWidth = self.ct_width / self.passwordLength ;
    for (int i = 0 ; i < self.dotsArray.count; i ++) {
        UIImageView *imageView = self.dotsArray[i];
        imageView.ct_width = imageWidth;
        imageView.ct_height = imageHeight;
        imageView.ct_centerY = self.ct_height * 0.5;
        imageView.ct_centerX = gridWidth * 0.5 + i * gridWidth;
    }
    for (int i = 0 ; i < self.labelsArray.count; i ++) {
        UILabel *label = self.labelsArray[i];
        label.ct_width = imageWidth;
        label.ct_height = imageHeight;
        label.ct_centerY = self.ct_height * 0.5;
        label.ct_centerX = gridWidth * 0.5 + i * gridWidth;
    }
...
{
    CTPasswordInputView *passwordInputView = [[self alloc] init];
...
- (void)refreshUI
{
    ...
    self.backgroundImageView.image = [UIImage ct_passwordInputGridImageWithGridCount:self.passwordLength gridLineColor:self.gridLineColor gridLineWidth:self.gridLineWidth];
    [self dotsArray];
    [self labelsArray];
